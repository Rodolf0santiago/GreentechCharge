...
export default function ConfiguracoesPage() {
  ...
  // Tab State
  const [activeSettingsTab, setActiveSettingsTab] = useState<'whatsapp' | 'landingPage'>('whatsapp');
  ...
  useEffect(() => {
    async function loadData() {
      try {
        ...
        const lpData = await getLandingPageData();
        setPortfolio(lpData.projects);
    ...
    <div className="min-h-screen bg-gray-50 text-gray-900 p-6 md:p-10 font-sans">
      <div className="max-w-4xl mx-auto space-y-8">
        ...
        <div className="flex border-b border-gray-200 gap-4 mb-4">
          ...
          <button
            onClick={() => setActiveSettingsTab('landingPage')}
            className={`px-5 py-3 text-sm font-bold border-b-2 transition-all cursor-pointer ${
              activeSettingsTab === 'landingPage'
                ? 'border-orange-500 text-orange-600'
                : 'border-transparent text-gray-500 hover:text-gray-900'
            }`}
          >
